...
import type { PatientSearchItem } from '@/lib/appointments/types';
...
export interface PatientLite {
  ...
  /** Maʼlum boʻlsa (tahrirlash / bemor kartasidan) — SMS koʻrsatmasi uchun */
  smsConsent?: boolean;
}
...
export interface PatientPickerProps {
  ...
  id?: string;
  invalid?: boolean;
  /** Tahrirlashda bemor oʻzgartirilmaydi */
  locked?: boolean;
}

const MIN_QUERY = 2;

function usePatientSearch(q: string, enabled: boolean) {
  return useQuery({
    queryKey: ['patients', 'search', { q, limit: 10 }],
    queryFn: () => api.get<{ items: PatientSearchItem[] }>(`/api/patients/search${qs({ q, limit: 10 })}`),
    enabled: enabled && q.trim().length >= MIN_QUERY,
    staleTime: 30_000,
  });
}

/** Bemor tanlash: server qidiruvi (ism / telefon / karta), yangi bemor yaratish havolasi */
export function PatientPicker({ value, onChange, disabled, id, invalid, locked }: PatientPickerProps) {
  const t = useT();
  ...
  if (value) {
    return (
      <div
        className={cn(
          'flex items-center gap-3 rounded-md border border-line bg-bg-elevated px-3 py-2',
          locked && 'opacity-90',
        )}
        id={id}
      >
        ...
        <Button asChild variant="ghost" size="sm" className="h-8 px-2 text-xs">
          <Link
            href={`/dashboard/patients/${value.id}`}
            target="_blank"
            rel="noopener"
            aria-label={t('appointments.dialog.openPatient')}
          >
        ...
        {!locked && !disabled ? (
          ...
          >
            <X aria-hidden="true" />
            {t('appointments.dialog.changePatient')}
          </Button>
        ) : null}
      </div>
    );
      ...
      >
        <Command shouldFilter={false} loop>
          <CommandInput
            ...
            placeholder={t('appointments.dialog.searchPatient')}
            aria-label={t('appointments.dialog.patient')}
          />
          <CommandList id={listId}>
            {tooShort ? (
              <div className="px-3 py-6 text-center text-sm text-text-muted">
                {t('appointments.dialog.typeToSearch')}
              </div>
            ) : search.isPending ? (
              ...
              <>
                <CommandEmpty>{t('appointments.dialog.patientNotFound')}</CommandEmpty>
                {items.length > 0 ? (
                  <CommandGroup>
                    {items.map((p) => (
                      <CommandItem
          ...
          <div className="border-t border-line p-1">
            ...
            >
              ...
              {t('appointments.dialog.createPatient')}
